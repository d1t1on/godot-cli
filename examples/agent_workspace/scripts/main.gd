...
func _ready() -> void:
	...
	if has_node("/root/WorkspaceState"):
		var workspace_state := get_node("/root/WorkspaceState")
		workspace_state.call("record_screen", "main")
		status_panel.call("set_status", "Ready", workspace_state.call("summary"))
...
func _on_counter_pressed() -> void:
	...
	if has_node("/root/WorkspaceState"):
		get_node("/root/WorkspaceState").call("increment_counter")
	status_panel.call("set_status", "Clicked %s" % count, {"count": count})
