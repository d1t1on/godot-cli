...
func save_slot(slot_id: String) -> Dictionary:
	...
	var result := SaveResult.make(true, normalized_slot_id)
	if normalized_slot_id.is_empty():
		SaveResult.add_error(result, "slot_id must be non-empty and may not contain path separators, ':', or '..'")
	...
	var payload := {
		"schema_version": SaveConstants.SCHEMA_VERSION,
		"module_version": SaveConstants.MODULE_VERSION,
		"saved_at": Time.get_datetime_string_from_system(true),
		"scene": _current_scene_path(),
		"participants": participants,
	}
	var dir_error := DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path(SaveConstants.SAVE_DIR))
	if dir_error != OK:
		SaveResult.add_error(result, "Could not create save directory: %s" % error_string(dir_error))
		save_failed.emit(normalized_slot_id, result)
		return result
	...
	if file == null:
		SaveResult.add_error(result, "Could not open save file for writing: %s" % error_string(FileAccess.get_open_error()))
		save_failed.emit(normalized_slot_id, result)
		return result
...
func load_slot(slot_id: String) -> Dictionary:
	...
	if not FileAccess.file_exists(path):
		SaveResult.add_error(result, "Save slot does not exist: %s" % normalized_slot_id)
	...
	if not payload is Dictionary:
		SaveResult.add_error(result, "Save file is not valid JSON object: %s" % normalized_slot_id)
		load_failed.emit(normalized_slot_id, result)
		return result
	if int(payload.get("schema_version", -1)) != SaveConstants.SCHEMA_VERSION:
		SaveResult.add_error(result, "Unsupported save schema_version: %s" % str(payload.get("schema_version")))
		load_failed.emit(normalized_slot_id, result)
		return result

	var saved_participants = payload.get("participants", {})
	if not saved_participants is Dictionary:
		SaveResult.add_error(result, "Save file participants field must be a Dictionary")
		load_failed.emit(normalized_slot_id, result)
		return result
	...
	for save_id in saved_participants.keys():
		...
		if not live_participants.has(key):
			SaveResult.add_warning(result, "Saved participant has no live node: %s" % key)
			continue
		var state = saved_participants[save_id]
		if not state is Dictionary:
			SaveResult.add_warning(result, "Saved participant state is not a Dictionary: %s" % key)
			continue
...
func delete_slot(slot_id: String) -> Dictionary:
	...
	if error != OK:
		SaveResult.add_error(result, "Could not delete save slot: %s" % error_string(error))
	return result


func list_slots() -> Array[Dictionary]:
	var slots: Array[Dictionary] = []
	var dir := DirAccess.open(SaveConstants.SAVE_DIR)
	if dir == null:
		return slots
	dir.list_dir_begin()
...
func _collect_participants(result: Dictionary, require_save_state: bool) -> Dictionary:
	...
	for save_id in nodes.keys():
		...
		if not state is Dictionary:
			SaveResult.add_error(result, "save_state() must return a Dictionary for %s" % save_id)
			continue
		if not _is_json_compatible(state):
			SaveResult.add_error(result, "save_state() returned non-JSON-compatible data for %s" % save_id)
			continue
...
func _participant_nodes_by_id(result: Dictionary, require_save_state: bool) -> Dictionary:
	var participants := {}
	for node in get_tree().get_nodes_in_group(SaveConstants.GROUP):
		if not is_instance_valid(node):
			continue
		if require_save_state and not node.has_method("save_state"):
			SaveResult.add_error(result, "Participant is missing save_state(): %s" % node.get_path())
			continue
		if not require_save_state and not node.has_method("load_state"):
			SaveResult.add_error(result, "Participant is missing load_state(data): %s" % node.get_path())
			continue
		var save_id := _resolve_save_id(node, result)
		if save_id.is_empty():
			continue
		if participants.has(save_id):
			SaveResult.add_error(result, "Duplicate save_id: %s" % save_id)
			continue
...
func _resolve_save_id(node: Node, result: Dictionary) -> String:
	...
	SaveResult.add_warning(result, "Participant uses NodePath fallback save_id: %s" % fallback)
	return fallback
...
func _slot_path(slot_id: String) -> String:
	return "%s/%s.json" % [SaveConstants.SAVE_DIR, slot_id]
